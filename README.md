...
This repository presents a collection of sorting algorithms implemented in **C**, **C++**, and **Python**, both for practice and as a resource for anyone preparing for relevant exams or interviews. Sections **[7](#7-counting-sort)**–**[9](#9-bucket-sort)** cover **linear-time** algorithms that are efficient when certain assumptions about the input hold (bounded key range, fixed-width digits, or roughly uniform distribution). For these three, the in-code comments have been expanded to explain each non-trivial step, making the logic easier to follow and debug.
